...
def build_image(apps, output_file, img_type="odroid", fatsize=0, target="unknown", version="unknown", fatimage=None):
    ...
    if fatsize:
        args += ["1", "129", fatsize, "vfs", fatimage or "none"]
...
parser.add_argument(
    "--fatimage", help="Populate the FAT storage partition from a prebuilt wear-leveling FAT image."
)
...
try:
    ...
    if command in ["build-fw", "release"]:
        ...
        if FW_FORMAT in ["odroid", "esplay"]:
            ...
            build_image(apps, fw_file, FW_FORMAT, args.fatsize, args.target, PROJECT_VER, args.fatimage)
        else:
    ...
    if command in ["build-img", "release", "install"]:
        ...
        build_image(apps, img_file, IDF_TARGET, args.fatsize, args.target, PROJECT_VER, args.fatimage)
