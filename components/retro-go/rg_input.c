...
static adc_oneshot_unit_handle_t adc_unit_handles[2];
#else
...
#ifdef ESP_PLATFORM
static inline int adc_get_raw(int unit, int channel)
{
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(6, 0, 0)
    int raw_value = -1;
    int index = unit == ADC_UNIT_1 ? 0 : (unit == ADC_UNIT_2 ? 1 : -1);
    if (index < 0 || !adc_unit_handles[index])
    {
        RG_LOGE("Invalid ADC unit %d", unit);
        return -1;
    }
    if (adc_oneshot_read(adc_unit_handles[index], channel, &raw_value) != ESP_OK)
    {
        RG_LOGE("ADC%d channel %d reading failed.", unit + 1, channel);
        return -1;
    }
    return raw_value;
#else
...
static inline bool adc_configure_channel(int unit, int channel, int atten)
{
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(6, 0, 0)
    int index = unit == ADC_UNIT_1 ? 0 : (unit == ADC_UNIT_2 ? 1 : -1);
    if (index < 0)
    ...
    if (!adc_unit_handles[index])
    {
        adc_oneshot_unit_init_cfg_t unit_config = {
            .unit_id = unit,
            .ulp_mode = ADC_ULP_MODE_DISABLE,
        };
        if (adc_oneshot_new_unit(&unit_config, &adc_unit_handles[index]) != ESP_OK)
        {
            RG_LOGE("Failed to initialize ADC unit %d!", unit);
            return false;
        }
    }
    ...
    if (adc_oneshot_config_channel(adc_unit_handles[index], channel, &channel_config) != ESP_OK)
    {
        RG_LOGE("Failed to configure ADC%d channel %d!", unit + 1, channel);
        return false;
    }
    return true;
#else
...
void rg_input_init(void)
{
    ...
    {
        const rg_keymap_adc_t *mapping = &keymap_adc[i];
        adc_configure_channel(mapping->unit, mapping->channel, mapping->atten);
    }
