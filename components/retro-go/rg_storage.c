...
bool rg_storage_read_file(const char *path, void **data_out, size_t *data_len, uint32_t flags)
{
    ...
    RG_LOGI("read_file: fopen start '%s'", path);
    FILE *fp = fopen(path, "rb");
    RG_LOGI("read_file: fopen returned %p errno=%d", fp, errno);
    if (!fp)
    {
        if (errno != ENOENT) // Only log unusual errors. Path not found isn't unusual.
    ...
    RG_LOGI("read_file: seek end '%s'", path);
    fseek(fp, 0, SEEK_END);
    file_size = ftell(fp);
    RG_LOGI("read_file: size=%u '%s'", (unsigned)file_size, path);
    fseek(fp, 0, SEEK_SET);
