...
{
    rg_bucket_t *bucket = rg_alloc(sizeof(rg_bucket_t) + capacity_bytes, MEM_FAST | MEM_NOPANIC);
    if (!bucket)
        return NULL;
...
#ifdef ESP_PLATFORM
    if ((caps & MEM_SLOW) && !(caps & (MEM_DMA | MEM_EXEC | MEM_32BIT)))
    {
        ptr = calloc(1, size);
        if (ptr)
        {
...
#else
    esp_caps |= (caps & MEM_EXEC ? MALLOC_CAP_INTERNAL : 0);
#endif
