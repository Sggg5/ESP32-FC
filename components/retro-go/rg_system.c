...
rg_app_t *rg_system_init(int sampleRate, const rg_handlers_t *handlers, void *_unused)
{
    ...
    RG_LOGI("System init: checking recovery button...");
    ...
    RG_LOGI("System init: initializing settings...");
    ...
    RG_LOGI("System init: ready to enter app.\n");
    ...
    if (app.lowMemoryMode)
        RG_LOGW("External memory not detected; continuing without blocking alert.");

    if (app.bootFlags & RG_BOOT_ONCE)
        update_boot_config(RG_APP_LAUNCHER, NULL, NULL, 0);

    RG_LOGI("System init: skipping system monitor task.");
    // rg_task_create("rg_sysmon", &system_monitor_task, NULL, 3 * 1024, RG_TASK_PRIORITY_5, -1);
    app.initialized = true;

    RG_LOGI("Available memory check skipped during startup.");
...
{
#if defined(ESP_PLATFORM)
    if (app && strcmp(app, "retro-core") == 0)
        return true;
