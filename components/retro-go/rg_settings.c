...
{
    RG_LOGI("Settings: json_root(%p, dirty=%d)", name, set_dirty);
    if (!config_root)
        return NULL;
    ...
    if (!branch)
    {
        ...
        if (!safe_mode)
        {
            ...
            RG_LOGI("Settings: reading '%s'", pathbuf);
            if (rg_storage_read_file(pathbuf, (void **)&data, &data_len, 0))
            {
                RG_LOGI("Settings: read %d bytes from '%s'", data_len, pathbuf);
            ...
            RG_LOGI("Settings: read attempt done for '%s'", pathbuf);
        }
    }
...
{
    RG_LOGI("Settings: init start safe=%d", _safe_mode);
    config_root = cJSON_CreateObject();
    RG_LOGI("Settings: root object ready");
    safe_mode = _safe_mode;
    json_root(NS_GLOBAL, 0);
    json_root(NS_BOOT, 0);
    RG_LOGI("Settings: init done");
}
