...
void gui_init(bool cold_boot)
{
    ...
    gui = (retro_gui_t){
        ...
        .language     = 0,
        .color_theme  = 0,
        .start_screen = START_SCREEN_BROWSER,
        .show_preview = PREVIEW_MODE_SAVE_COVER,
        .scroll_mode  = SCROLL_MODE_CENTER,
...
tab_t *gui_add_tab(const char *name, const char *desc, void *arg, void *event_handler)
{
    ...
    tab_t *tab = rg_alloc(sizeof(tab_t), MEM_FAST);
    RG_ASSERT(tab, "tab allocation failed");
    ...
    tab->listbox = (listbox_t){
        .items = rg_alloc(10 * sizeof(listbox_item_t), MEM_FAST),
        .capacity = 10,
        .length = 0,
        .cursor = 0,
        .sort_mode = SORT_TEXT_ASC,
    };
    RG_ASSERT(tab->listbox.items, "listbox allocation failed");
