...
#ifdef RG_ENABLE_NETWORKING
    RG_LOGI("Launcher: skipping network_init.");
#endif

    if (!gui_get_current_tab())
        gui.selected_tab = 0;
    RG_LOGI("Launcher: selecting tab...");
    tab = gui_set_current_tab(gui.selected_tab);
    RG_LOGI("Launcher: loop ready.");
    ...
    while (true)
    {
        ...
        if (change_tab || gui.browse != browse_last)
        {
            ...
            if (gui.browse)
            {
                if (!tab->initialized)
                {
                    RG_LOGI("Launcher: init current tab...");
                    gui_init_tab(tab);
                    RG_LOGI("Launcher: init current tab done.");
                }
        ...
        if (redraw_pending)
        {
            ...
            RG_LOGI("Launcher: redraw...");
            gui_redraw();
            RG_LOGI("Launcher: redraw done.");
        }
...
void app_main(void)
{
    ...
    RG_LOGI("Launcher: system init returned.");
    app->configNs = "launcher";
    app->isLauncher = true;

    if (!rg_storage_ready())
    {
        RG_LOGW("Storage mount failed; continuing without SD card.");
        while (true)
        {
            int64_t start_time = rg_system_timer();
            rg_display_clear(C_BLACK);
            rg_gui_draw_text(RG_GUI_CENTER, RG_GUI_CENTER, 0, "SD card not mounted", C_WHITE, C_BLACK, RG_TEXT_BIGGER);
            rg_system_tick(rg_system_timer() - start_time);
            rg_task_delay(1000);
        }
    }
    ...
    RG_LOGI("Launcher: entering main loop.");
    retro_loop();
}
