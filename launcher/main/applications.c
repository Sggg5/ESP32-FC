...
static void application(const char *desc, const char *name, const char *exts, const char *part, uint16_t crc_offset)
{
    ...
    RG_LOGI("application: registering '%s' (%s)", name, part);
    RG_LOGI("application: checking partition '%s'", part);
    bool have_app = rg_system_have_app(part);
    RG_LOGI("application: partition check result=%d", have_app);
    if (!have_app)
    {
    ...
    RG_LOGI("application: allocating app struct");
    retro_app_t *app = calloc(1, sizeof(retro_app_t));
    RG_LOGI("application: app=%p", app);
    RG_ASSERT(app, "app allocation failed");
    apps[apps_count++] = app;

    RG_LOGI("application: filling fields");
    ...
    RG_LOGI("application: marking available");
    app->available = true;
    RG_LOGI("application: allocating files (%d bytes)", (int)(APP_FILE_CAPACITY * sizeof(retro_file_t)));
    app->files = rg_alloc(APP_FILE_CAPACITY * sizeof(retro_file_t), MEM_FAST);
    RG_LOGI("application: files=%p", app->files);
    RG_ASSERT(app->files, "files allocation failed");
    app->files_capacity = APP_FILE_CAPACITY;
    RG_LOGI("application: creating filename bucket");
    app->filenames = rg_bucket_create(4096);
    RG_LOGI("application: filenames=%p", app->filenames);
    RG_ASSERT(app->filenames, "filename bucket allocation failed");
    app->crc_offset = crc_offset;

    RG_LOGI("application: adding gui tab");
    gui_add_tab(app->short_name, app->description, app, event_handler);
    RG_LOGI("application: registered '%s' done", name);
    rg_task_delay(1);
}

void applications_init(void)
{
    RG_LOGI("applications_init: registering systems...");
    application("Nintendo Entertainment System", "nes", "nes fc fds nsf zip", "retro-core", 16);
    RG_LOGI("applications_init: NES registered.");
    RG_LOGI("applications_init: single-app flash mode.");
    return;
    ...
    RG_LOGI("applications_init: registered %d apps.", apps_count);

    // Internal flash can be slow on first boot; CRC cache is optional and can
    // be rebuilt on demand from the game info screen.
    // if (!rg_system_get_app()->lowMemoryMode)
    //     crc_cache_init();
    RG_LOGI("applications_init: done.");
}
