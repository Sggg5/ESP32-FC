...
static uint8 microwire_latch;

static uint8 microwire_read(uint32 address)
{
    return ((address >> 8) & 0xFE) | microwire_latch;
}

static void microwire_write(uint32 address, uint8 value)
{
    microwire_latch = value & 1;
}

static void map_write(uint32 address, uint8 value)
{
    address = (address & 0xF000) | (address & 0x000F);
...
mapintf_t map23_intf =
{
    ...
    .set_state  = NULL,
    .mem_read   = {
        { 0x6000, 0x6FFF, microwire_read },
    },
    .mem_write  = {
        { 0x6000, 0x6FFF, microwire_write },
