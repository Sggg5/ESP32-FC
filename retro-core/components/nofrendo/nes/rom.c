...
rom_t *rom_loadmem(uint8 *data, size_t size)
{
   ...
   if (!memcmp(data, ROM_NES_MAGIC, 4))
   {
      ...
      const db_game_t *entry = NULL;
      for (size_t i = 0; games_database[i].crc; i++)
      {
         if (games_database[i].crc == rom.checksum)
         {
            entry = &games_database[i];
            break;
         }
      }
...
rom_t *rom_loadfile(const char *filename)
{
   ...
   if ((fp = fopen(filename, "rb")))
   {
      ...
      MESSAGE_INFO("ROM: seek end...\n");
      fseek(fp, 0, SEEK_END);
      size = ftell(fp);
      MESSAGE_INFO("ROM: file size is %ld bytes\n", size);
      fseek(fp, 0, SEEK_SET);
      ...
      if (data)
      {
         ...
         {
            size_t todo = (size_t)size - total;
            if (todo > chunk)
               todo = chunk;

            size_t got = fread(data + total, 1, todo, fp);
            if (got != todo)
            {
               MESSAGE_ERROR("ROM: Read error at %d/%ld, got %d\n", (int)total, size, (int)got);
               free(data);
               data = NULL;
               break;
            }
            ...
            total += got;
            if ((total & 0x7FFF) == 0 || total == (size_t)size)
               MESSAGE_INFO("ROM: read %d/%ld bytes\n", (int)total, size);
            vTaskDelay(1);
         }
      }
      else if (size >= 16 && size <= 0x200000)
      {
         MESSAGE_ERROR("ROM: Memory allocation failed\n");
      }
      fclose(fp);
   ...
   {
      MESSAGE_ERROR("ROM: fopen failed for '%s'\n", filename);
   }
