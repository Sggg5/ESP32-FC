...
esp_err_t LcdDisplay::PauseLvglRefresh() {
...
esp_err_t LcdDisplay::ResumeLvglRefresh() {
    if (!lvgl_port_lock(3000)) {
        return ESP_ERR_TIMEOUT;
    }
...
esp_err_t LcdDisplay::DrawBitmapBlocking(int x_start, int y_start, int x_end, int y_end,
                                         const void* color_data) {
    esp_err_t result = esp_lcd_panel_draw_bitmap(panel_, x_start, y_start, x_end, y_end,
                                                  color_data);
    if (result != ESP_OK) {
        return result;
    }
    // A parameter transaction drains queued SPI color transactions, making the
    // single DMA strip safe to reuse without replacing LVGL's completion callback.
    return esp_lcd_panel_io_tx_param(panel_io_, 0x00, nullptr, 0);
}

void* LcdDisplay::GetLvglDrawBuffer(size_t* size_bytes) {
    lv_draw_buf_t* draw_buffer = lv_display_get_buf_active(display_);
    if (!draw_buffer) {
        if (size_bytes) *size_bytes = 0;
        return nullptr;
    }
    if (size_bytes) *size_bytes = draw_buffer->data_size;
    return draw_buffer->data;
}
